    ...
    <section className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      {placements.length === 0 ? (
        ...
        placements.map((placement) => (
          <div key={placement._id} className="bg-white rounded-lg shadow-sm border border-neutral-200 overflow-hidden p-6">
            <h2 className="text-lg font-semibold text-neutral-800 mb-4">{placement.year} Placements</h2>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-neutral-200">
                <thead className="bg-gray-100">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-700">Department</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-700">Approved Intake</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-700">Companies Visited</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-700">Students Placed</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-700">Avg Package</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {placement.departments.map((department, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      <td className="px-6 py-4 text-sm text-gray-900">{department.department}</td>
                      <td className="px-6 py-4 text-sm text-gray-900">{department.approvedIntake}</td>
                      <td className="px-6 py-4 text-sm text-gray-900">{department.companiesVisited}</td>
                      <td className="px-6 py-4 text-sm text-gray-900">{department.studentsPlaced}</td>
                      <td className="px-6 py-4 text-sm text-gray-900">₹ {department.avgPackage} LPA</td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center p-4">
          <div className="bg-white p-6 rounded-lg w-full max-w-md">
            ...
            <input
              ...
              onChange={(e) =>
                setNewPlacement({
                  ...newPlacement,
                  departments: [
                    { ...newPlacement.departments[0], department: e.target.value },
                  ],
                })
              }
              className="w-full mb-2 p-2 border rounded"
            ...
            <input
              ...
              onChange={(e) =>
                setNewPlacement({
                  ...newPlacement,
                  departments: [
                    { ...newPlacement.departments[0], approvedIntake: e.target.value },
                  ],
                })
              }
              className="w-full mb-2 p-2 border rounded"
            ...
            <input
              ...
              onChange={(e) =>
                setNewPlacement({
                  ...newPlacement,
                  departments: [
                    { ...newPlacement.departments[0], companiesVisited: e.target.value },
                  ],
                })
              }
              className="w-full mb-2 p-2 border rounded"
            ...
            <input
              ...
              onChange={(e) =>
                setNewPlacement({
                  ...newPlacement,
                  departments: [
                    { ...newPlacement.departments[0], studentsPlaced: e.target.value },
                  ],
                })
              }
              className="w-full mb-2 p-2 border rounded"
            ...
            <input
              ...
              onChange={(e) =>
                setNewPlacement({
                  ...newPlacement,
                  departments: [
                    { ...newPlacement.departments[0], avgPackage: e.target.value },
                  ],
                })
              }
            ...
            <div className="flex justify-end space-x-3">
              <button onClick={() => setIsDialogOpen(false)} className="px-4 py-2 bg-gray-300 rounded">
                Cancel
              </button>
              <button onClick={handleAddPlacement} className="px-4 py-2 bg-green-600 text-white rounded">
                Add
              </button>
            </div>
          </div>
