...
const CollegePoliciesPage = () => {
	...
	// Function to update policy content
	const updatePolicyContent = (policyId, newContent) => {
	...
	return (
		<div className="min-h-screen bg-gray-50 lg:py-16 px-4 lg:px-8">
			<div className="max-w-4xl mx-auto">
				<div className="mb-12">
					<h1 className="text-3xl text-center font-bold mb-2 text-[#98002E]">
						College Policies & Guidlines
					</h1>
					...
					<div key={policy.id} className="mb-4 border rounded-lg shadow-sm">
						<button
							className={`w-full px-6 py-4 text-left flex justify-between items-center ${
								openSections[policy.id] ? 'bg-[#ffe4ec]' : 'bg-white'
							} hover:bg-[#ffe4ec] transition-colors rounded-t-lg`}
							onClick={() => toggleSection(policy.id)}>
							<h2 className="text-xl font-semibold text-[#98002E]">{policy.title}</h2>
							<svg
								className={`w-6 h-6 transform transition-transform text-[#98002E] ${
									openSections[policy.id] ? 'rotate-180' : ''
								}`}
								fill="none"
								stroke="currentColor"
								viewBox="0 0 24 24">
